...
class BaseModule(nn.Module):
	...
	def save_embeddings(self, path):
		a = self.get_parameters('list')
		embbedding_list = a["ent_embeddings.weight"]
		relation_list = a["rel_embeddings.weight"]
		entity_path = path + 'entity2vec.bern'
		relation_path = path + 'relation2vec.bern'
		f = open(entity_path, 'w')
		for i in embbedding_list:
			for j in i:
				f.writelines(str(j) + ' ')
			f.writelines('\n')
		f.close()
		f = open(relation_path, 'w')
