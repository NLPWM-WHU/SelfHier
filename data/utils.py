...
def gen_mapping(args):
    ...
    for rel in relations.keys():
        ...
        num += 1
    ent_lines.insert(0, str(len(ent_lines))+'\n')
    rel_lines.insert(0, str(len(rel_lines))+'\n')
...
def gen_eval(train_triples, valid_triples, test_triples, relation2id):
    ...
    for valid_triple in valid_triples:
        h, r, t = valid_triple
        valid_id.append(str(h) + ' ' + str(t) + ' ' + str(r) + '\n')#这个地方用(h,t,r)是为了方便openke产生embeddings
    ...
    for test_triple in test_triples:
        h, r, t = test_triple
        test_id.append(str(h) + ' ' + str(t) + ' ' + str(r) + '\n') #这个地方用(h,t,r)是为了方便openke产生embeddings
        test_line.append(str(h)+'\t'+'R'+str(r)+'\t'+str(t)+'\n')
        test_line_rev.append(str(t)+'\t'+'R'+str(r+rel_num)+'\t'+str(h)+'\n')
    test_id.insert(0,str(len(test_id))+'\n')
    valid_id.insert(0, str(len(valid_id))+'\n')
    with open(os.path.join(args.dataset, 'valid2id.txt'), 'w') as f:
        f.writelines(valid_id)
    ...
    with open(os.path.join(args.dataset, 'test2id.txt'), 'w') as f:
        f.writelines(test_id)
    ...
    for triple in train_triples:
        h, r, t = triple
        train_id.append(str(h)+' '+str(t)+' '+str(r)+'\n')#这个地方用(h,t,r)是为了方便openke产生embeddings
        train_line.append(str(h)+'\t'+'R'+str(r)+'\t'+str(t)+'\n')
        train_line_rev.append(str(t)+'\t'+'R'+str(r+rel_num)+'\t'+str(h)+'\n')
    train_id.insert(0,str(len(train_id))+'\n')
    with open(os.path.join(args.dataset, 'train2id.txt'), 'w') as f:
